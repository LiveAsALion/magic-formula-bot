...
def get_magic_formula_picks():
    ...
    try:
        # Step 1: Get S&P 500 list with a "Badge" (User-Agent) to avoid 403
        url = 'https://en.wikipedia.org/wiki/List_of_S%26P_500_companies'
        headers = {'User-Agent': 'MagicFormulaBot/1.0 (Contact: your-email@example.com)'}
        
        response = requests.get(url, headers=headers)
        table = pd.read_html(response.text, attrs={'id': 'constituents'})[0]
        tickers = table['Symbol'].str.replace('.', '-', regex=True).tolist()
        
        # Analyze top 50 for the initial scan
        universe = tickers[:50]
        scored_data = []
        ...
        for symbol in universe:
            try:
                ...
                ey = ebitda / ev if ev > 0 else 0
                
                # Quality proxy (ROA)
                roa = info.get('returnOnAssets', 0)
                
                if ey > 0 and roa > 0:
                    scored_data.append({'ticker': symbol, 'ey': ey, 'roa': roa})
        ...
        df['rank'] = df['ey'].rank(ascending=False) + df['roa'].rank(ascending=False)
        
        picks = df.sort_values('rank').head(15)['ticker'].tolist()
        print(f"✅ Found {len(picks)} candidates.")
        return picks
    except Exception as e:
        print(f"⚠️ Screening Error: {e}")
        return []

# --- 3. TREND FILTER & EXECUTION ---
...
def run_strategy():
    ...
    if not candidates:
        send_telegram_msg("⚠️ **System Error**: Failed to fetch S&P 500 list (403 or Parse Error).")
        return

    final_picks = [t for t in candidates if is_above_200_ma(t)][:5]
    send_telegram_msg(f"💓 **Heartbeat**: 💚 System Healthy. Found {len(final_picks)} trending picks.")
...
if __name__ == "__main__":
    run_strategy()pos = trading_client.get_open_position(ticker)
            trading_client.submit_order(TrailingStopOrderRequest(
                symbol=ticker, qty=pos.qty, side=OrderSide.SELL, time_in_force=TimeInForce.GTC, trail_percent=TRAIL_PERCENT
            ))
            summary += f"✅ **{ticker}**\n"
